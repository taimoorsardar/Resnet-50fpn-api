...
class ImageProcessor:
    ...
    def __init__(self, cats, state_dict_path, model="net.resnet50_cam",
        ...
        # If GPU is available, set the GPU device, otherwise it will default to CPU
    ...
    def execute_cams_pred(self, image):
        ...
        with torch.no_grad():
            ...
            self.__cam_pred_model.to(self.device)
            image_labels = torch.from_numpy(np.ones(self.num_cats)).to(self.device)
            valid_cat = torch.nonzero(image_labels, as_tuple=True)[0]
            strided_up_size = get_strided_up_size(image_size, 16)
            results = [self.__cam_pred_model(t.to(self.device, non_blocking=True))
                       for t in scaled_tensors]
    ...
    def __load_model(self, class_name):
        ...
        model.load_state_dict(torch.load(self.state_dict_path,weights_only=True), strict=True)
